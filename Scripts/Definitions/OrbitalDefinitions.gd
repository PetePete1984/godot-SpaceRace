...
var orbital_types = [
	"shipyard", "missiles", "dock", "cloaker", "orbital_shield", "orbital_mega_shield", "ship_placeholder", "short_whopper", "long_whopper"
]
...
func get_name(def_key):
	if orbital_defs[def_key].research_name != null:
		return orbital_defs[def_key].research_name
	return orbital_defs[def_key].orbital_name